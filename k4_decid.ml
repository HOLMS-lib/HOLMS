(* ========================================================================= *)
(* Decision procedure for the modal logic K4.                                *)
(*                                                                           *)
(* (c) Copyright, Antonella Bilotta, Marco Maggesi,                          *)
(*                Cosimo Perini Brogi 2025.                                  *)
(* ========================================================================= *)

needs "HOLMS/k4_completeness.ml";;
needs "HOLMS/gen_countermodel.ml";;

(* ------------------------------------------------------------------------- *)
(* Lemmata.                                                                  *)
(* ------------------------------------------------------------------------- *)

let IN_TF_CLAUSES = prove
 (`!W:W->bool R.
     (W,R) IN TF
     ==> (!x y z. R y z ==> R x y ==> R x z) /\
         (!w w'. R w w'
                 ==> !p. holds (W,R) V (Box p) w ==> holds (W,R) V p w') /\
         (!w p. holds (W,R) V (Box p) w
                 ==> !w'. R w w' ==> holds (W,R) V p w')`,
  REWRITE_TAC[IN_TF; IN_FINITE_FRAME; TRANSITIVE] THEN
  MESON_TAC[HOLDS_LEFT_BOX]);;

let K4_COMPLETENESS_NUM =
 let K4_COMPLETENESS_THEOREM_NUM =
   REWRITE_RULE[num_INFINITE]
     (INST_TYPE [`:num`,`:A`] K4_COMPLETENESS_THM_GEN) in
 prove
 (`!p. (!W:num->bool R.
          (W,R) IN TF
          ==> !V w. w IN W ==> holds (W,R) V p w)
       ==> [K4_AX . {} |~ p]`,
  GEN_TAC THEN DISCH_TAC THEN MATCH_MP_TAC K4_COMPLETENESS_THEOREM_NUM THEN
  REWRITE_TAC[valid; FORALL_PAIR_THM; holds_in] THEN
  INTRO_TAC "![W] [R]; rf" THEN REPEAT STRIP_TAC THEN
  ASM_MESON_TAC[IN_TF_CLAUSES]);;

(* ------------------------------------------------------------------------- *)
(* Top-level invocation.                                                     *)
(* ------------------------------------------------------------------------- *)

let IN_TF_RULES =
  let tm = `(W:num->bool,R) IN TF` in
  let th = MATCH_MP IN_TF_CLAUSES (ASSUME tm) in
  CONJUNCTS th;;

let K4_TAC : tactic =
  GEN_HOLMS_TAC MATCH_BOX_RIGHT_TAC K4_COMPLETENESS_NUM IN_TF_RULES;;

holms_register_tactic `K4_AX` K4_TAC;;

(* ------------------------------------------------------------------------- *)
(* Tests.                                                                    *)
(* ------------------------------------------------------------------------- *)

(*
let PREPARE_TAC = HOLMS_PREPARE_TAC K4_COMPLETENESS_NUM;;
let SATURATE_TAC = HOLMS_SATURATE_TAC IN_TF_RULES;;
let STEP_TAC = HOLMS_STEP_TAC SATURATE_TAC;;
let INNER_TAC = INNER_HOLMS_TAC SATURATE_TAC;;
let box_right_tac = BOX_RIGHT_THEN SATURATE_TAC;;
let right_tac = HOLMS_RIGHK4_TAC SATURATE_TAC;;
*)

g `[K4_AX . {} |~ Box (Atom a && Atom b) <->
                  Box Atom a && Box Atom b]`;;
e K4_TAC;;
top_thm();;

(* ------------------------------------------------------------------------- *)
(* Countermodels.                                                            *)
(* ------------------------------------------------------------------------- *)

let K4_HOLMS_CERTIFY_COUNTERMODEL : term -> term -> thm =
  let ltm = `TF:(num->bool)#(num->num->bool)->bool` in
  fun ctm tm ->
    let fm = rand (snd (strip_forall tm))
    and eth = mk_countermodel_existence_thm ctm in
    prove (mk_not_valid_ptm ltm fm,
           CERTIFY_COUNTERMODEL_TAC eth);;

(* ------------------------------------------------------------------------- *)
(* Experiments.                                                              *)
(* ------------------------------------------------------------------------- *)

(* Axioms *)
HOLMS_RULE `!a. [K4_AX . {} |~ Box a --> Box Box a]`;;

(* ------------------------------------------------------------------------- *)
(* Non-axiom examples.                                                       *)
(* ------------------------------------------------------------------------- *)

(* T_AX *)
let tm = `!a. [{} . {} |~ Box a --> a]`;;
let ctm = HOLMS_BUILD_COUNTERMODEL tm;;
K4_HOLMS_CERTIFY_COUNTERMODEL ctm tm;;

(* B_AX *)
let tm = `!a. [{} . {} |~ a --> Box Diam a]`;;
let ctm = HOLMS_BUILD_COUNTERMODEL tm;;
K4_HOLMS_CERTIFY_COUNTERMODEL ctm tm;;

(* S5_AX *)
let tm = `!a. [{} . {} |~ Diam a --> Box Diam  a]`;;
let ctm = HOLMS_BUILD_COUNTERMODEL tm;;
K4_HOLMS_CERTIFY_COUNTERMODEL ctm tm;;

(* GL_AX *)
(* 
let tm = `!a. [{} . {} |~ Box (Box a --> a) --> Box a]`;;
let ctm = HOLMS_BUILD_COUNTERMODEL tm;;
K4_HOLMS_CERTIFY_COUNTERMODEL ctm tm;;
*)

(* ------------------------------------------------------------------------- *)
(* Simple examples.                                                          *)
(* ------------------------------------------------------------------------- *)

let tm = `!a. [K4_AX . {} |~ a --> Box a]`;;
let ctm = HOLMS_BUILD_COUNTERMODEL tm;;
K4_HOLMS_CERTIFY_COUNTERMODEL ctm tm;;

let tm = `!a. [K4_AX . {} |~ a --> Box Box a]`;;
let ctm = HOLMS_BUILD_COUNTERMODEL tm;;
K4_HOLMS_CERTIFY_COUNTERMODEL ctm tm;;

needs "Library/iter.ml";;

let tm = `!a. [K4_AX . {} |~ a --> ITER 5 (Box) a]`;;
let tm = run_conv (TOP_SWEEP_CONV num_CONV THENC REWRITE_CONV [ITER]) tm;;
let ctm = HOLMS_BUILD_COUNTERMODEL tm;;
time (K4_HOLMS_CERTIFY_COUNTERMODEL ctm) tm;;   (* CPU time (user): 0.657422 *)
