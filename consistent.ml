...
let CONSISTENT_ALT = prove
 (`CONSISTENT S (l:form list) <=> ~[S . {} |~ Not (CONJLIST l)]`,
  REWRITE_TAC[CONSISTENT; MLK_not_def;
              MODPROVES_DEDUCTION_LEMMA_CONJLIST_EMPTY]);;

let CONSISTENT_IFF_SETCONSISTENT = prove
 (`!S X. CONSISTENT S X <=> SETCONSISTENT S (set_of_list X)`,
  REWRITE_TAC[SETCONSISTENT; CONSISTENT]);;

let CONSISTENT_SING = prove
 (`!S p. CONSISTENT S [p] <=> ~[S . {} |~ Not p]`,
  REWRITE_TAC[CONSISTENT_ALT; CONJLIST]);;
...
let CONSISTENT_SUBLIST = prove
 (`!S X Y. CONSISTENT S X /\ Y SUBLIST X ==> CONSISTENT S Y`,
  REPEAT GEN_TAC THEN REWRITE_TAC[CONSISTENT_ALT] THEN
...
let NOT_CONSISTENT_2 = prove
 (`!S p q X. MEM p X /\ MEM q X /\ [S . {} |~ Not (p && q)] ==> ~CONSISTENT S X`,
  ...
  [ASM_REWRITE_TAC[CONSISTENT_ALT; CONJLIST; NOT_CONS_NIL]; ALL_TAC] THEN
...
let CONSISTENT_CONS = prove
 (`!S h t. CONSISTENT S (CONS h t) <=> ~[S . {} |~ Not h || Not CONJLIST t]`,
  REPEAT GEN_TAC THEN REWRITE_TAC[CONSISTENT_ALT] THEN AP_TERM_TAC THEN
...
let MAXIMAL_CONSISTENT_LEMMA = prove
 (`!S p X A b. MAXIMAL_CONSISTENT S p X /\
  ...
  ASM_REWRITE_TAC[CONSISTENT_ALT; MLK_not_def]);;
...
(* TODO: To be removed. *)
let MAXIMAL_CONSISTENT_SUBFORMULA_MEM_EQ_DERIVABLE_OLD = prove
 (`!S p w q. MAXIMAL_CONSISTENT S p w /\ q SUBFORMULA p
...
let MAXIMAL_CONSISTENT_SUBFORMULA_MEM_NEG_EQ_DERIVABLE = prove
 (`!S p w q. MAXIMAL_CONSISTENT S p w /\ q SUBFORMULA p
  ...
  REPEAT GEN_TAC THEN REWRITE_TAC[MAXIMAL_CONSISTENT; CONSISTENT_ALT] THEN
