...
let S4_UNION = prove
 (`S4_AX = T_AX UNION K4_AX`,
  REWRITE_TAC[S4_AX;T_AX; K4_AX;EXTENSION] THEN
  REWRITE_TAC[UNION;IN_ELIM_THM; IN_UNIV] THEN MESON_TAC[]);; 

let FOUR_IN_S4_AX = prove
 (`!q. Box q --> Box Box q IN S4_AX`,
  REWRITE_TAC[S4_AX; IN_ELIM_THM; IN_UNIV; UNION] THEN MESON_TAC[]);;

let T_IN_S4_AX = prove
 (`!q. Box q -->  q IN S4_AX`,
  REWRITE_TAC[S4_AX; IN_ELIM_THM; IN_UNIV; UNION] THEN MESON_TAC[]);;

let S4_AX_FOUR = prove
 (`!q. [S4_AX. {} |~ (Box q --> Box Box q)]`,
  MESON_TAC[MODPROVES_RULES; FOUR_IN_S4_AX]);;

let S4_AX_T = prove
 (`!q. [S4_AX. {} |~ (Box q --> q)]`,
  MESON_TAC[MODPROVES_RULES; T_IN_S4_AX]);;

let S4_DOT_BOX = prove
 (`!p. [S4_AX . {} |~ (Box p --> Box p && Box (Box p))]`,
  MESON_TAC[MLK_imp_refl_th; S4_AX_FOUR; MLK_and_intro]);;

(* ------------------------------------------------------------------------- *)
(* Reflexive-Transitive frames.                                              *)
(* ------------------------------------------------------------------------- *)

let RTRANS_DEF = new_definition
  `RTRANS =
   {(W:W->bool,R:W->W->bool) |
    ~(W = {}) /\
...
let IN_RTRANS = prove
 (`(W:W->bool,R:W->W->bool) IN RTRANS <=>
   ...
   (!x:W. x IN W ==> R x x) /\
   (!x y z:W. x IN W /\ y IN W /\ z IN W /\ R x y /\ R y z ==> R x z)`,
  REWRITE_TAC[RTRANS_DEF; IN_ELIM_PAIR_THM]);;

(* ------------------------------------------------------------------------- *)
(* Correspondence Theory: Refl-Transtive Frames are characteristic for S4.   *)
(* ------------------------------------------------------------------------- *)

g `RTRANS:(W->bool)#(W->W->bool)->bool = CHAR S4_AX`;;
e (REWRITE_TAC[EXTENSION; FORALL_PAIR_THM]);;
e (REWRITE_TAC[IN_CHAR; IN_RTRANS; IN_FRAME]);;
e (REPEAT GEN_TAC);;
e EQ_TAC;;
 e (INTRO_TAC "not_empty Rel Refl Trans");;
 e (ASM_REWRITE_TAC[S4_UNION]);;
  e (GEN_TAC THEN INTRO_TAC "DISJ" THEN  FIRST_ASSUM (fun th -> DISJ_CASES_TAC (REWRITE_RULE[IN_UNION;T_AX; IN_ELIM_THM; K4_AX] th)));;
    e (CLAIM_TAC "@q. form T" `? p'. p' IN (:form) /\ p = Box p' --> p'`);;
      e (ASM_REWRITE_TAC[]);;
    e (ASM_MESON_TAC[MODAL_REFL]);;
    e (CLAIM_TAC "@q. form 4" `? p'. p' IN (:form) /\ p = Box p' --> Box Box p'`);;
      e (ASM_REWRITE_TAC[]);;
    e (ASM_MESON_TAC[MODAL_TRANS]);;
 e (INTRO_TAC "(not_empty Rel) char");;
  e (ASM_MESON_TAC[MODAL_REFL; MODAL_TRANS; T_IN_S4_AX; FOUR_IN_S4_AX]);;
let RTRANS_CHAR_S4 = top_thm();;

(* ------------------------------------------------------------------------- *)
(* Proof of soundness w.r.t. Reflexive-Transitive Frames                     *)
(* ------------------------------------------------------------------------- *)

let S4_RTRANS_VALID = prove
 (`!H p. [S4_AX . H |~ p] /\
         (!q. q IN H ==> RTRANS:(W->bool)#(W->W->bool)->bool |= q)
         ==> RTRANS:(W->bool)#(W->W->bool)->bool |= p`,
  ASM_MESON_TAC[GEN_CHAR_VALID; RTRANS_CHAR_S4]);;

(* ------------------------------------------------------------------------- *)
(* Finite Reflexive-Transitive Frames are appropriate for S4.                *)
(* ------------------------------------------------------------------------- *)
...
let IN_RTF = prove
 (`(W:W->bool,R:W->W->bool) IN RTF <=>
   ...
   (!x. x IN W ==> R x x) /\
   (!x y z:W. x IN W /\ y IN W /\ z IN W /\ R x y /\ R y z ==> R x z)`,
  REWRITE_TAC[RTF_DEF; IN_ELIM_PAIR_THM]);;

let RTF_SUBSET_RTRANS = prove
 (`RTF:(W->bool)#(W->W->bool)->bool SUBSET RTRANS`,
  REWRITE_TAC[SUBSET; FORALL_PAIR_THM; IN_RTF; IN_RTRANS] THEN MESON_TAC[]);;

(*let RTF_SUBSET_REFL = prove
 (`RTF:(W->bool)#(W->W->bool)->bool SUBSET REFL`,
  REWRITE_TAC[SUBSET; FORALL_PAIR_THM; IN_RTF; IN_REFL] THEN MESON_TAC[]);;
  
let RTF_SUBSET_TRANS = prove
 (`RTF:(W->bool)#(W->W->bool)->bool SUBSET TRANS`,
  REWRITE_TAC[SUBSET; FORALL_PAIR_THM; IN_RTF; IN_TRANS] THEN MESON_TAC[]);;*)
...
e EQ_TAC;;
 e (INTRO_TAC "In_RTF");;
 e CONJ_TAC;;
  e (ASM_MESON_TAC [RTF_SUBSET_RTRANS; SUBSET; RTRANS_CHAR_S4]);;
  e (HYP_TAC "In_RTF" (REWRITE_RULE[IN_RTF]));;
   e (ASM_REWRITE_TAC[]);;
 e (INTRO_TAC "In_Char Fin");;
  e (SUBGOAL_THEN  `(p1:W->bool,p2:W->W->bool) IN RTRANS` MP_TAC);;
   e (ASM_MESON_TAC[RTRANS_CHAR_S4; EXTENSION; FORALL_PAIR_THM]);;
  e (ASM_REWRITE_TAC[IN_RTRANS; IN_RTF]);;
let RTF_APPR_S4 = top_thm();;


(* ------------------------------------------------------------------------- *)
(* Proof of soundness w.r.t. RTF.                                            *)
(* ------------------------------------------------------------------------- *)

let S4_RTF_VALID = prove
 (`!p. [S4_AX . {} |~ p] ==> RTF:(W->bool)#(W->W->bool)->bool |= p`,
  MESON_TAC[GEN_APPR_VALID; RTF_APPR_S4]);;

(* ------------------------------------------------------------------------- *)
(* Proof of Consistency of S4.                                               *)
(* ------------------------------------------------------------------------- *)
...
let S4_STANDARD_FRAME_DEF = new_definition
  `S4_STANDARD_FRAME p = GEN_STANDARD_FRAME S4_AX p`;;

let IN_S4_STANDARD_FRAME = prove
  (`!p W R. (W,R) IN S4_STANDARD_FRAME p <=>
            W = {w | MAXIMAL_CONSISTENT S4_AX p w /\
                     (!q. MEM q w ==> q SUBSENTENCE p)} /\
            (W,R) IN RTF /\
            (!q w. Box q SUBFORMULA p /\ w IN W
                   ==> (MEM (Box q) w <=> !x. R w x ==> MEM q x))`,
 REPEAT GEN_TAC THEN
 REWRITE_TAC[S4_STANDARD_FRAME_DEF; IN_GEN_STANDARD_FRAME] THEN
 EQ_TAC THEN MESON_TAC[RTF_APPR_S4]);;

(* ------------------------------------------------------------------------- *)
(* Standard model.                                                           *)
(* ------------------------------------------------------------------------- *)

let S4_STANDARD_MODEL_DEF = new_definition
  `S4_STANDARD_MODEL = GEN_STANDARD_MODEL S4_AX`;;

let RTF_SUBSET_FRAME = prove
 (`RTF:(W->bool)#(W->W->bool)->bool SUBSET FRAME`,
  REWRITE_TAC[SUBSET; FORALL_PAIR_THM] THEN INTRO_TAC "![W] [R]" THEN
  REWRITE_TAC[IN_RTF] THEN STRIP_TAC THEN ASM_REWRITE_TAC[IN_FRAME]);;

let S4_STANDARD_MODEL_CAR = prove
 (`!W R p V.
     S4_STANDARD_MODEL p (W,R) V <=>
     (W,R) IN S4_STANDARD_FRAME p /\
     (!a w. w IN W ==> (V a w <=> MEM (Atom a) w /\ Atom a SUBFORMULA p))`,
  REPEAT GEN_TAC THEN
  REWRITE_TAC[S4_STANDARD_MODEL_DEF; GEN_STANDARD_MODEL_DEF] THEN
  REWRITE_TAC[IN_S4_STANDARD_FRAME; IN_GEN_STANDARD_FRAME] THEN
  EQ_TAC THEN ASM_MESON_TAC[RTF_APPR_S4]);;

(* ------------------------------------------------------------------------- *)
(* Truth Lemma.                                                              *)
(* ------------------------------------------------------------------------- *)

let S4_TRUTH_LEMMA = prove
 (`!W R p V q.
     ~ [S4_AX . {} |~ p] /\
     S4_STANDARD_MODEL p (W,R) V /\
     q SUBFORMULA p
     ==> !w. w IN W ==> (MEM q w <=> holds (W,R) V q w)`,
  REWRITE_TAC[S4_STANDARD_MODEL_DEF] THEN MESON_TAC[GEN_TRUTH_LEMMA]);;
...
let S4_STANDARD_REL_CAR = prove
 (`!p w x.
     S4_STANDARD_REL p w x <=>
     MAXIMAL_CONSISTENT S4_AX p w /\ (!q. MEM q w ==> q SUBSENTENCE p) /\
     MAXIMAL_CONSISTENT S4_AX p x /\ (!q. MEM q x ==> q SUBSENTENCE p) /\
     (!B. MEM (Box B) w ==> MEM (Box B) x /\ MEM B x)`,
  REPEAT GEN_TAC THEN REWRITE_TAC[S4_STANDARD_REL_DEF; GEN_STANDARD_REL] THEN
  EQ_TAC THEN REPEAT (ASM_MESON_TAC[]) THEN REPEAT (ASM_MESON_TAC[]));;


 let RTF_MAXIMAL_CONSISTENT = prove
 (`!p. ~ [S4_AX . {} |~ p]
       ==> ({M | MAXIMAL_CONSISTENT S4_AX p M /\
  ...
  EXISTS_TAC `S4_AX` THEN EXISTS_TAC `p:form` THEN EXISTS_TAC `[Box B]` THEN
  ...
  [CLAIM_TAC "c" `Box B SUBSENTENCE p` THENL [ASM_MESON_TAC[]; ALL_TAC] THEN
   ...
   ASM_REWRITE_TAC[];
   ALL_TAC] THEN
  ASM_REWRITE_TAC[CONJLIST] THEN ASM_MESON_TAC[S4_AX_T]; ALL_TAC] THEN
  (* Transitive *)
  REWRITE_TAC[IN_ELIM_THM; S4_STANDARD_REL_CAR] THEN
  INTRO_TAC "!x y z; (x1 x2) (y1 y2) (z1 z2) +" THEN
  ASM_REWRITE_TAC[] THEN ASM_MESON_TAC[]);; 

let CONJLIST_FLATMAP_DOT_BOX_LEMMA_S4 = prove
 (`!w. [S4_AX . {} |~
  ...
  MATCH_ACCEPT_TAC S4_DOT_BOX);;
  
g `!p w q.
     ~ [S4_AX . {} |~ p] /\
     MAXIMAL_CONSISTENT S4_AX p w /\
     (!q. MEM q w ==> q SUBSENTENCE p) /\
     Box q SUBFORMULA p /\
     (!x. S4_STANDARD_REL p w x ==> MEM q x)
     ==> MEM (Box q) w`;;
...
e (CLAIM_TAC "consistent_X"
  ...
  e CONJ_TAC;;
   ...
   e (EXISTS_TAC `CONJLIST (MAP (Box)
                  (FLATMAP (\x. match x with Box c -> [c; Box c] | _ -> []) w))`);;
   e CONJ_TAC;;
    e (MP_TAC (CONJLIST_FLATMAP_DOT_BOX_LEMMA_S4));;
    e (ASM_MESON_TAC[]);;
   e (CLAIM_TAC "XIMP" `!y l. [S4_AX . {} |~ Not (Not y && CONJLIST l)]
                                ==> [S4_AX . {} |~ (CONJLIST (MAP (Box) l)) --> Box(y)]`);;
    e (REPEAT STRIP_TAC);;
    e (MATCH_MP_TAC MLK_imp_trans);;
  ...
  e (COND_CASES_TAC);;
   ...
   e (INTRO_TAC "notnotq");;
   e (CLAIM_TAC "or" `[S4_AX . {} |~ Not Not q || Not CONJLIST []]`);;
     e (ASM_MESON_TAC [MLK_or_introl]);;
   e (CLAIM_TAC "eq" `[S4_AX . {} |~ Not (Not q && CONJLIST []) <-> Not Not q || Not CONJLIST []]`);;
...
e ANTS_TAC;;
 ...
 e (ASM_REWRITE_TAC[S4_STANDARD_REL_DEF]);;
 e (MP_TAC (ISPECL [`S4_AX`; `p:form`;`w: form list`; `q:form`]
...
let S4_ACCESSIBILITY_LEMMA = top_thm();;

(* ------------------------------------------------------------------------- *)
(* Modal completeness theorem for S4.                                        *)
(* ------------------------------------------------------------------------- *)
...
g `!p. ~ [S4_AX . {} |~ p]
       ==> ({M | MAXIMAL_CONSISTENT S4_AX p M /\
                 (!q. MEM q M ==> q SUBSENTENCE p)},
            S4_STANDARD_REL p)
           IN S4_STANDARD_FRAME p`;;
e (INTRO_TAC "!p; not_theor_p");;
e (REWRITE_TAC [IN_S4_STANDARD_FRAME]);;
e CONJ_TAC;;
 e (MATCH_MP_TAC RTF_MAXIMAL_CONSISTENT);;
 e (ASM_REWRITE_TAC[]);;
 e (ASM_REWRITE_TAC[IN_ELIM_THM]);;
e (INTRO_TAC "!q w; boxq maxw subw");;
e EQ_TAC;;
 e (ASM_MESON_TAC[S4_STANDARD_REL_CAR]);;
e (ASM_MESON_TAC[S4_ACCESSIBILITY_LEMMA]);;
let S4F_IN_S4_STANDARD_FRAME = top_thm();;

let S4_COUNTERMODEL = prove
 (`!M p.
     ~ [S4_AX . {} |~ p] /\
     MAXIMAL_CONSISTENT S4_AX p M /\
     MEM (Not p) M /\
     (!q. MEM q M ==> q SUBSENTENCE p)
     ==>
     ~holds
  ...
  EXISTS_TAC `S4_AX` THEN ASM_REWRITE_TAC[GEN_STANDARD_MODEL_DEF] THEN
  CONJ_TAC THENL
  [ASM_MESON_TAC[S4F_IN_S4_STANDARD_FRAME; S4_STANDARD_FRAME_DEF];
   ALL_TAC] THENL
  [ASM_MESON_TAC[IN_ELIM_THM]]);;

g `!p. RTF:(form list->bool)#(form list->form list->bool)->bool |= p
       ==> [S4_AX . {} |~ p]`;;
e (GEN_TAC THEN GEN_REWRITE_TAC I [GSYM CONTRAPOS_THM]);;
e (INTRO_TAC "p_not_theor");;
e (REWRITE_TAC[valid; NOT_FORALL_THM]);;
e (EXISTS_TAC `({M | MAXIMAL_CONSISTENT S4_AX p M /\
                     (!q. MEM q M ==> q SUBSENTENCE p)},
                S4_STANDARD_REL p)`);;
e (REWRITE_TAC[NOT_IMP] THEN CONJ_TAC);;
e (MATCH_MP_TAC RTF_MAXIMAL_CONSISTENT);;
e (ASM_REWRITE_TAC[]);;
e (SUBGOAL_THEN `({M | MAXIMAL_CONSISTENT S4_AX p M /\
                       (!q. MEM q M ==> q SUBSENTENCE p)},
                  S4_STANDARD_REL p)
                 IN GEN_STANDARD_FRAME S4_AX p`
                MP_TAC);;
 e (ASM_MESON_TAC[S4F_IN_S4_STANDARD_FRAME; S4_STANDARD_FRAME_DEF]);;
e (ASM_MESON_TAC[GEN_COUNTERMODEL_ALT]);;
let S4_COMPLETENESS_THM = top_thm ();;


(* ------------------------------------------------------------------------- *)
(* Modal completeness for S4 for models on a generic (infinite) domain.      *)
(* ------------------------------------------------------------------------- *)

let S4_COMPLETENESS_THM_GEN = prove
 (`!p. INFINITE (:A) /\ RTF:(A->bool)#(A->A->bool)->bool |= p
       ==> [S4_AX . {} |~ p]`,
  SUBGOAL_THEN
    `INFINITE (:A)
     ==> !p. RTF:(A->bool)#(A->A->bool)->bool |= p
             ==> RTF:(form list->bool)#(form list->form list->bool)->bool |= p`
    (fun th -> MESON_TAC[th; S4_COMPLETENESS_THM]) THEN
  ASM_MESON_TAC[RTF_APPR_S4; GEN_LEMMA_FOR_GEN_COMPLETENESS]);;

(* ------------------------------------------------------------------------- *)
(* Simple decision procedure for S4.                                         *)
(* ------------------------------------------------------------------------- *)

let S4_TAC : tactic =
  MATCH_MP_TAC S4_COMPLETENESS_THM THEN
  REWRITE_TAC[valid; FORALL_PAIR_THM; holds_in; holds;
              IN_RTF; GSYM MEMBER_NOT_EMPTY] THEN
  MESON_TAC[];;

let S4_RULE tm =
  prove(tm, REPEAT GEN_TAC THEN S4_TAC);;

S4_RULE `!p q r. [S4_AX . {} |~ p && q && r --> p && r]`;;
S4_RULE `!p. [S4_AX . {} |~ Box p --> Box (Box p)]`;;
S4_RULE `!p q. [S4_AX . {} |~  Box (p --> q) && Box p --> Box q]`;;
S4_RULE `!p q. [S4_AX . {} |~  Box p --> p]`;;
(*S4_RULE `!p. [S4_AX . {} |~ (Box (Box p --> p) --> Box p)]`;;*)
(*S4_RULE `!p. [S4_AX . {} |~ Box (Box p --> p) --> Box p]`;; *)
(*S4_RULE `[S4_AX . {} |~ Box (Box False --> False) --> Box False]`;;*)

(* ------------------------------------------------------------------------- *)
(* Countermodel using set of formulae (instead of lists of formulae).        *)
(* ------------------------------------------------------------------------- *)

let S4_STDWORLDS_RULES,S4_STDWORLDS_INDUCT,S4_STDWORLDS_CASES =
  new_inductive_set
  `!M. MAXIMAL_CONSISTENT S4_AX p M /\
       (!q. MEM q M ==> q SUBSENTENCE p)
       ==> set_of_list M IN S4_STDWORLDS p`;;

let S4_STDREL_RULES,S4_STDREL_INDUCT,S4_STDREL_CASES = new_inductive_definition
  `!w1 w2. S4_STANDARD_REL p w1 w2
           ==> S4_STDREL p (set_of_list w1) (set_of_list w2)`;;

let S4_STDREL_IMP_S4_STDWORLDS = prove
 (`!p w1 w2. S4_STDREL p w1 w2 ==>
             w1 IN S4_STDWORLDS p /\
             w2 IN S4_STDWORLDS p`,
  GEN_TAC THEN MATCH_MP_TAC S4_STDREL_INDUCT THEN
  REWRITE_TAC[S4_STANDARD_REL_CAR] THEN REPEAT STRIP_TAC THEN
  MATCH_MP_TAC S4_STDWORLDS_RULES THEN ASM_REWRITE_TAC[]);;

let SET_OF_LIST_EQ_S4_STANDARD_REL = prove
 (`!p u1 u2 w1 w2.
     set_of_list u1 = set_of_list w1 /\ NOREPETITION w1 /\
     set_of_list u2 = set_of_list w2 /\ NOREPETITION w2 /\
     S4_STANDARD_REL p u1 u2
     ==> S4_STANDARD_REL p w1 w2`,
  REPEAT GEN_TAC THEN REWRITE_TAC[S4_STANDARD_REL_CAR] THEN
  STRIP_TAC THEN
  CONJ_TAC THENL
...
let S4_BISIMIMULATION_SET_OF_LIST = prove
(`!p. BISIMIMULATION
      (
        {M | MAXIMAL_CONSISTENT S4_AX p M /\
             (!q. MEM q M ==> q SUBSENTENCE p)},
 ...
 [INTRO_TAC "![u1]; w1u1" THEN EXISTS_TAC `set_of_list u1:form->bool` THEN
  HYP_TAC "w1u1 -> hp" (REWRITE_RULE[S4_STANDARD_REL_CAR]) THEN
  ASM_REWRITE_TAC[] THEN CONJ_TAC THENL
  [MATCH_MP_TAC S4_STDWORLDS_RULES THEN ASM_REWRITE_TAC[];
   ALL_TAC] THEN
  CONJ_TAC THENL
  [MATCH_MP_TAC S4_STDWORLDS_RULES THEN ASM_REWRITE_TAC[];
   ALL_TAC] THEN
  FIRST_X_ASSUM SUBST_VAR_TAC THEN MATCH_MP_TAC S4_STDREL_RULES THEN
  ASM_REWRITE_TAC[];
  ALL_TAC] THEN
 INTRO_TAC "![u2]; w2u2" THEN EXISTS_TAC `list_of_set u2:form list` THEN
 REWRITE_TAC[CONJ_ACI] THEN
 HYP_TAC "w2u2 -> @x2 y2. x2 y2 x2y2" (REWRITE_RULE[S4_STDREL_CASES]) THEN
 ...
 CONJ_TAC THENL
 [HYP_TAC "x2y2 -> hp" (REWRITE_RULE[S4_STANDARD_REL_CAR]) THEN
  ASM_REWRITE_TAC[];
  ALL_TAC] THEN
 CONJ_TAC THENL
 [ASM_MESON_TAC[S4_STDREL_IMP_S4_STDWORLDS]; ALL_TAC] THEN
 CONJ_TAC THENL
 ...
 ASM_MESON_TAC[S4_STANDARD_REL_CAR];
 ALL_TAC] THEN
 MATCH_MP_TAC SET_OF_LIST_EQ_S4_STANDARD_REL THEN
...
let S4_COUNTERMODEL_FINITE_SETS = prove
 (`!p. ~ [S4_AX . {} |~ p] ==> ~holds_in (S4_STDWORLDS p, S4_STDREL p) p`,
  INTRO_TAC "!p; p" THEN
  DESTRUCT_TAC "@M. max mem subf"
    (MATCH_MP NONEMPTY_MAXIMAL_CONSISTENT (ASSUME `~ [S4_AX . {} |~ p]`)) THEN
  REWRITE_TAC[holds_in; NOT_FORALL_THM; NOT_IMP] THEN
  ASSUM_LIST (LABEL_TAC "hp" o MATCH_MP S4_COUNTERMODEL o
  ...
  [MATCH_MP_TAC S4_STDWORLDS_RULES THEN ASM_REWRITE_TAC[]; ALL_TAC] THEN
  REMOVE_THEN "hp" MP_TAC THEN
  MATCH_MP_TAC (MESON[] `(p <=> q) ==> (~p ==> ~q)`) THEN
  MATCH_MP_TAC BISIMIMULATION_HOLDS THEN
  EXISTS_TAC`\w1 w2. MAXIMAL_CONSISTENT S4_AX p w1 /\
                     (!q. MEM q w1 ==> q SUBSENTENCE p) /\
                     w2 IN S4_STDWORLDS p /\
                     set_of_list w1 = w2` THEN
  ASM_REWRITE_TAC[S4_BISIMIMULATION_SET_OF_LIST] THEN
  MATCH_MP_TAC S4_STDWORLDS_RULES THEN ASM_REWRITE_TAC[]);;
