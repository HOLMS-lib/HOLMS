...
let MODAL_SER = prove
 (`!W:W->bool R.
  ...
  REWRITE_TAC[diam_DEF; D_SCHEMA_DEF] THEN
  MODAL_SCHEMA_TAC THEN MESON_TAC[SERIAL]);;

let MODAL_REFL = prove
 (`!W:W->bool R.
     REFLEXIVE W R <=>
     (!p. holds_in (W,R) (T_SCHEMA p))`,
  REWRITE_TAC[REFLEXIVE; T_SCHEMA_DEF] THEN
  MODAL_SCHEMA_TAC THEN MESON_TAC[]);;

let MODAL_TRANS = prove
 (`!W:W->bool R.
     TRANSITIVE W R <=>
     (!p. holds_in (W,R) (4_SCHEMA p))`,
...
let MODAL_SYM = prove
 (`!W:W->bool R.
  ...
  ASM_REWRITE_TAC[diam_DEF; SYMETRIC; B_SCHEMA_DEF] THEN MODAL_SCHEMA_TAC THEN
